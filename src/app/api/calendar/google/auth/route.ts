...
import { createGoogleCalendarService } from "@/lib/calendar/google";
  ...
  try {
    ...
    if (existingIntegration?.is_active) {
      return NextResponse.json({ 
        ok: false, 
        error: "Google Calendar integration already exists and is active" 
      }, { status: 400 });
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Set credentials and get user's calendar list to verify connection
    ...
    if (!calendars || calendars.length === 0) {
      return NextResponse.json({ 
        ok: false, 
        error: "No calendars found. Please check your Google Calendar permissions." 
      }, { status: 400 });
    }
    ...
    const { data: integration, error: integrationError } = await supa
      ...
      .upsert({
        ...
        is_active: true,
        sync_enabled: true,
        display_name: displayName || 'Google Calendar',
    ...
    return NextResponse.json({ 
      ok: false, 
      error: error.message || "Failed to complete Google Calendar integration" 
    }, { status: 500 });
  }
}