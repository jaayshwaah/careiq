...
export default function ChatWindow({
  ...
  const sendHint = isMac ? (
    <span className="text-xs text-gray-600 dark:text-white/50">
      Press <kbd>Return</kbd> to send • <kbd>Shift</kbd>+<kbd>Return</kbd> for
      newline
    </span>
  ) : (
    <span className="text-xs text-gray-600 dark:text-white/50">
  ...
  // ----- EMPTY / NEW CHAT VIEW (centered) -----
  if (!chat || (messages?.length ?? 0) === 0) {
    return (
      <div className="flex h-full flex-col">
        {/* Center the stack in the viewport */}
        <div className="flex-1 grid place-content-center">
          <div className="mx-auto w-full max-w-2xl px-6">
            ...
            <form
              ...
              className="mt-6 glass ring-1 ring-black/10 dark:ring-white/10 rounded-2xl p-3 shadow-soft focus-within:ring-2 focus-within:ring-black/20 dark:focus-within:ring-white/20 transition"
...
function Suggestions({ onSend }: { onSend: (t: string) => void }) {
  ...
  const blurs = useMemo(
    () => choices.map(() => 12 + Math.floor(Math.random() * 8)),
    []
  );
        ...
        {choices.map((t, i) => (
          <div key={t} className="relative inline-block group">
            <span
              ...
              style={{
                background: `linear-gradient(120deg, ${palettes[i][0]}, ${palettes[i][1]})`,
...
function MessageBubble({ m }: { m: Message }) {
  ...
  return (
    <div className={`mb-4 flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[78%] rounded-2xl px-4 py-2 text-[15px] leading-relaxed shadow-soft ${bubbleClasses}`}
      >
        <div className="whitespace-pre-wrap">{m.content}</div>
        <div className={`mt-1 text-[11px] ${metaClasses}`}>
          {timeAgo(m.createdAt)}
        </div>
      </div>
    </div>
