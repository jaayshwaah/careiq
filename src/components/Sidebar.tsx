...
export default function Sidebar({ chats = [], collapsedByDefault, user }: Props) {
  ...
  const [collapsed, setCollapsed] = React.useState<boolean>(() => {
    ...
    return collapsedByDefault ?? isHome; // default collapsed on home
  });
  ...
  React.useEffect(() => {
    ...
    setCollapsed(pathname === "/"); // collapse on home, expand elsewhere
    setShowSearch(false);
    setSearchTerm("");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]);

  // Clicking anywhere on a collapsed sidebar expands it
    ...
    >
      ...
      {collapsed ? (
        <div className="flex h-full flex-col items-center justify-between py-3">
          {/* Top stack: CareIQ icon, New Chat, Search */}
          <div className="flex flex-col items-center gap-3">
            ...
            <button
              ...
              aria-label="New Chat"
              onClick={(e) => {
                e.stopPropagation(); // prevent expanding twice
                handleNewChat();
              }}
            >
